...
# Safely tokenize INPUT_SHELLCHECK_FLAGS into an array (quote-aware splitting via xargs)
shellcheck_flags=()
_shellcheck_flags_default='--external-sources'
_shellcheck_flags_input="${INPUT_SHELLCHECK_FLAGS:-${_shellcheck_flags_default}}"
if [ -n "${_shellcheck_flags_input}" ]; then
  while IFS= read -r -d '' t; do shellcheck_flags+=("$t"); done \
    < <(printf '%s' "${_shellcheck_flags_input}" | xargs printf '%s\0')
fi

# Safely tokenize INPUT_REVIEWDOG_FLAGS into an array (quote-aware splitting via xargs)
reviewdog_flags=()
